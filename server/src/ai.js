  ...
  const contents = [
    {
      ...
      parts: [
        {
          text: [
            'Create a high-quality PNG of an original pokemon from pokemon series based on this doodle.',
...
export async function generatePokemonMeta(promptText, { baseImageDataUrl, apiKey } = {}) {
  ...
  const parts = [
    {
      text: [
        promptText || 'Design a new original pokemon from pokemon series based on the provided reference.',
...
export async function generateActionImage({ baseImageDataUrl, name, type, characteristics, apiKey }, power) {
  ...
  const actionText = [
    `Create a high-quality PNG of the same original pokemon from pokemon series performing the action "${power?.name || 'Unknown'}".`,
