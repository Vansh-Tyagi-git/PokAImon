...
const availableTypes = useMemo(() => {
  const types = new Set(processedData.map((p) => p.type).filter(Boolean));
  ...
  const deleteItem = async (id) => {
    try {
      const res = await fetch(`${API}/api/pokaimon/${id}/delete`, {
        method: "DELETE",
      });
            ...
            processedData.map((p) => (
              <div key={p.id} className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
                ...
                <button
                  ...
                  className="mt-3 w-full px-3 py-2 bg-red-600 text-white rounded-md hover:bg-red-700"
                >
                  Delete
                </button>
              </div>
